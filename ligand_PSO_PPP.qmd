---
title: "Ligand Receptors Activity For PSO vs PPP"
---
...
```{r}
ligand_colors <- c(
  "B_cell" = "#ff7f0e",              # A warm orange
  "CAF-specific" = "#9467bd",        # A moderate purple
  "DC" = "#2ca02c",                  # A balanced green
  "Endothelial-specific" = "#17becf", # A soft cyan
  "Epithelial_cells" = "#214245",    # A dark teal (kept original)
  "Fibroblasts" = "#d62728",         # A distinct red
  "Fibroblasts-specific" = "#bcbd22",# A muted olive
  "General" = "#1f77b4",             # A calm blue
  "Keratinocytes" = "#8c564b",       # A neutral brown
  "Keratinocytes-specific" = "#e377c2",  # A light pink
  "Macrophage" = "#7f7f7f",          # A neutral gray
  "Monocyte" = "#c5b0d5",            # A pastel purple
  "Monocyte-specific" = "#ff9896",   # A light coral
  "NK_cell" = "#c49c94",             # A subtle tan
  "Neutrophils" = "#f7b6d2",         # A soft pink
  "T_cells" = "#dbdb8d",             # A soft yellow-green
  "T_cells-specific" = "#ffbb78",    # A light orange
...
```{r}
make_circos_plot_transparency_workaround <- function (vis_circos_obj, transparency = FALSE, args.circos.text = list(), 
    ...) 
{
    ...
    # Normalize weights and set a minimum threshold for transparency
    transparency_val <- vis_circos_obj$links_circle %>%
        ...
        mutate(transparency = ifelse(transparency > 0.8, 0.8, transparency)) %>%
        .$transparency
}

    default_params <- list(x = vis_circos_obj$links_circle, order = vis_circos_obj$order, 
        grid.col = vis_circos_obj$ligand_colors, transparency = transparency_val, 
        directional = 1, link.sort = TRUE, link.decreasing = FALSE, 
        diffHeight = 0.005, direction.type = c("diffHeight", 
            "arrows"), link.arr.type = "big.arrow", link.visible = vis_circos_obj$links_circle$weight >= 
            attr(vis_circos_obj$links_circle, "cutoff_include_all_ligands"), 
    ...
    circos_text_default_params <- list(facing = "clockwise", 
        niceFacing = TRUE, adj = c(0, 0.55), cex = 1)
...
```{r}
# Set the desired minimum value for the weights
#min_threshold <- 1.0

# Normalize the weights to range from the minimum threshold to 1
#vis_circos_obj$links_circle$weight <- scales::rescale(vis_circos_obj$links_circle$weight, to = c(min_threshold, 1))
```
...
```{r}
make_circos_plot_transparency_workaround(vis_circos_obj, transparency = TRUE,  args.circos.text = list(cex = 0.5)) 
```
